...
import (
	...
	"sigs.k8s.io/addon-operators/discovery/controllers"
)
...
func main() {
	...
	reconciler, err := controllers.NewAddonReconciler(
		mgr.GetClient(),
		ctrl.Log.WithName("controllers").WithName("Addon"),
		mgr.GetScheme(),
	)
	if err != nil {
		setupLog.Error(err, "unable to create reconciler", "controller", "Addon")
		os.Exit(1)
	}
